        ...
        for _section in ["scaffold", "abstract", "introduction", "related work", "methods",
            ...
            if _section in ["introduction", "related work", "methods", "conclusion"]:
                ...
                while len(papers) == 0:  # 如果检索不到文献就重试（最多5次）
                    ...
                    search_query = query_model(
                        ...
                        prompt=f"Given the following research topic {self.topic} and research plan: \n\n{self.plan}\n\nPlease come up with a search query to find relevant papers on PubMed. Respond only with the search query and nothing else. This should be a a string that will be used to find papers with semantically similar content. {att_str}",
                    ...
                    search_query.replace('"', '')  # 去除多余引号
                    papers = PubMed_eng.search_papers(query=search_query)  # 检索前10篇相关文献
                    first_attempt = False
                    attempts += 1
                if len(papers) != 0:
            ...
            while not section_complete:
                ...
                if _section == "references":
                    references_list = []
                    for sec, papers in self.section_related_work.items():
                        for paper in papers:
                            ...
                            references_list.append(
                                f"\\bibitem{{PubMed{pubmed_id}}} {authors}. {title}. {journal}. {year}."
                            )
...
\\end{figure}\n"""
            ...
            # \\label{fig:fig1}
            # \\end{figure}\n"""
            fig_dir = "research_results/results/figures"
